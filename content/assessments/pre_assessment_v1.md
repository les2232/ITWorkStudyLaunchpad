# IT Work Study Launchpad Pre-Assessment v1

## Purpose

This assessment helps determine each new IT work-study student's starting knowledge, confidence level, and recommended training path.

It is not pass/fail. The goal is to understand what the student already knows so they can receive the right training and support.

## Student Intro

Welcome to the IT Work Study Launchpad pre-assessment.

This is not a test you pass or fail. It helps us understand your current comfort level with IT concepts, troubleshooting, documentation, and escalation. Your answers help us recommend the right training path and give your mentor a better idea of how to support you.

Answer honestly. It is completely okay if some of these topics are new to you.

---

## Categories

1. Background / Experience
2. Confidence
3. IT Vocabulary
4. Beginner Troubleshooting
5. Workflow and Escalation
6. Work Style and Role Alignment

---

## Scoring

Total: 100 points

The app auto-scores multiple-choice, select-all, and rating questions. Free-response answers are mentor-review items and are not auto-scored. The percentage shown in the app reflects only auto-scorable points.

Work Style and Role Alignment is reported separately from the 100-point technical readiness score. It is not a pass/fail fit test and should be used as a planning signal for support and placement conversations.

| Category | Points |
|---|---:|
| Background / Experience | 10 |
| Confidence | 10 |
| IT Vocabulary | 25 |
| Beginner Troubleshooting | 25 |
| Workflow and Escalation | 30 |
| Work Style and Role Alignment | Separate signal |

---

## Readiness Levels

| Score | Level | Recommended Path |
|---:|---|---|
| 0-39 | Beginner | Full IT Foundations path |
| 40-69 | Developing | Foundations + Day 1 checklist |
| 70-84 | Ready to Shadow | Day 1 checklist + shadowing |
| 85-100 | Advanced Beginner | Faster path, still complete required modules |

---

# Questions

## Section 1: Background / Experience

### Q1. Have you worked in an IT, computer lab, help desk, repair, or tech support role before?

**Type:** Multiple choice  
**Category:** Background / Experience  
**Points:** 3

A. Yes  
B. No  
C. Somewhat  

**Scoring Guide:**

| Answer | Points |
|---|---:|
| A | 3 |
| C | 2 |
| B | 1 |

---

### Q2. Which of these have you done before? Select all that apply.

**Type:** Select all that apply  
**Category:** Background / Experience  
**Points:** 4

- Connected monitors, keyboards, or mice
- Installed software
- Helped someone troubleshoot a computer
- Used a ticketing system
- Worked with inventory or asset tags
- Imaged or reset a computer
- Worked in customer service
- None of these

**Scoring Guide:**

Award up to 4 points based on relevant prior exposure.

| Response Pattern | Points |
|---|---:|
| 5 or more relevant items selected | 4 |
| 3-4 relevant items selected | 3 |
| 1-2 relevant items selected | 2 |
| None of these | 1 |

---

### Q3. What are you most nervous about in this role?

**Type:** Free response  
**Category:** Background / Experience  
**Points:** 3

**Mentor Review Guide:**

This question is not auto-scored. Review the answer separately for honesty, support needs, and useful first-check-in context.

**Mentor Use:**

Use this answer to identify early support needs, confidence concerns, and topics to explain during the first shadowing session.

---

## Section 2: Confidence

Use this scale:

1 = Not confident  
2 = Slightly confident  
3 = Somewhat confident  
4 = Confident  
5 = Very confident  

---

### Q4. How confident do you feel using computers in general?

**Type:** Rating  
**Category:** Confidence  
**Points:** 2.5

---

### Q5. How confident do you feel troubleshooting basic computer problems?

**Type:** Rating  
**Category:** Confidence  
**Points:** 2.5

---

### Q6. How confident do you feel asking for help when you are stuck?

**Type:** Rating  
**Category:** Confidence  
**Points:** 2.5

---

### Q7. How confident do you feel reading step-by-step technical documentation?

**Type:** Rating  
**Category:** Confidence  
**Points:** 2.5

**Scoring Guide for Q4-Q7:**

Convert each 1-5 rating into points.

| Rating | Points |
|---:|---:|
| 1 | 0.5 |
| 2 | 1 |
| 3 | 1.5 |
| 4 | 2 |
| 5 | 2.5 |

---

## Section 3: IT Vocabulary

### Q8. In a school or workplace IT environment, what is a domain?

**Type:** Multiple choice  
**Category:** IT Vocabulary  
**Points:** 5

A. A website address only  
B. A managed network environment used for accounts, computers, and access  
C. A type of monitor cable  
D. A folder on the desktop  

**Correct Answer:** B

**Explanation:**

In this context, a domain helps an organization manage user accounts, computers, permissions, and access in a central way.

---

### Q9. What does computer imaging mean?

**Type:** Multiple choice  
**Category:** IT Vocabulary  
**Points:** 5

A. Taking a picture of a computer  
B. Preparing a computer with the required operating system, software, and settings  
C. Printing a computer label  
D. Changing the desktop background  

**Correct Answer:** B

**Explanation:**

Computer imaging prepares a computer with the standard operating system, software, settings, and configuration it needs before being used.

---

### Q10. SCCM or Configuration Manager is generally used for what?

**Type:** Multiple choice  
**Category:** IT Vocabulary  
**Points:** 5

A. Managing and deploying software, updates, and operating systems  
B. Designing websites  
C. Replacing broken screens  
D. Sending email  

**Correct Answer:** A

**Explanation:**

Many IT departments use Configuration Manager, often still called SCCM, to manage computers, deploy software, run updates, and help with computer imaging.

---

### Q11. What is a ticket?

**Type:** Multiple choice  
**Category:** IT Vocabulary  
**Points:** 5

A. A record of a request, issue, task, or support interaction  
B. A password  
C. A computer cable  
D. A storage device  

**Correct Answer:** A

**Explanation:**

A ticket is a record of work that needs to be done or support that was provided. Good ticket notes or mentor summaries help other IT staff understand what happened.

---

### Q12. What is an asset tag?

**Type:** Multiple choice  
**Category:** IT Vocabulary  
**Points:** 5

A. A label or ID used to track equipment  
B. A Wi-Fi password  
C. A username  
D. A software update  

**Correct Answer:** A

**Explanation:**

An asset tag is used to identify and track equipment such as computers, monitors, printers, and other technology.

---

## Section 4: Beginner Troubleshooting

### Q13. A monitor says "No Signal." What should you check first?

**Type:** Multiple choice  
**Category:** Beginner Troubleshooting  
**Points:** 6

A. Reimage the computer  
B. Check power, display cable, monitor input/source, and whether the computer is on  
C. Delete the user profile  
D. Replace the computer immediately  

**Correct Answer:** B

**Explanation:**

A "No Signal" message is often related to power, cabling, monitor input/source, or whether the computer is actually sending video. These are beginner-safe checks.

---

### Q14. A keyboard is not working. What is a beginner-safe first step?

**Type:** Multiple choice  
**Category:** Beginner Troubleshooting  
**Points:** 6

A. Open the computer case  
B. Check the connection or try a known-working keyboard  
C. Reinstall Windows  
D. Change domain settings  

**Correct Answer:** B

**Explanation:**

Checking the connection or testing with a known-working keyboard is a safe first step. Opening the computer, reinstalling Windows, or changing system settings should not be done without approval.

---

### Q15. Which of these is usually beginner-safe?

**Type:** Multiple choice  
**Category:** Beginner Troubleshooting  
**Points:** 6

A. Guessing which image to use  
B. Changing system settings without approval  
C. Checking whether a monitor cable is connected  
D. Deleting old user folders  

**Correct Answer:** C

**Explanation:**

Checking a cable connection is usually safe. Guessing, changing settings, or deleting files can cause problems and should require approval.

---

### Q16. A computer is running slowly. What is the safest beginner response?

**Type:** Multiple choice  
**Category:** Beginner Troubleshooting  
**Points:** 7

A. Delete files until the computer is faster  
B. Document the symptoms, ask what the user was doing, and check beginner-safe items if instructed  
C. Reimage the computer immediately  
D. Ignore the issue because slow computers are normal  

**Correct Answer:** B

**Explanation:**

The safest beginner response is to document what is happening, collect basic context, perform only approved beginner-safe checks, and escalate when needed.

---

## Section 5: Workflow and Escalation

### Q17. You are asked to help image a computer, but you are not sure which image or workflow to use. What should you do?

**Type:** Multiple choice  
**Category:** Workflow and Escalation  
**Points:** 7

A. Pick the one that seems closest  
B. Stop and ask your mentor or the IT tech on duty  
C. Skip the imaging step  
D. Mark the computer complete  

**Correct Answer:** B

**Explanation:**

A new work-study student should not guess during imaging. If you are unsure which image or workflow applies, stop and ask your mentor or the IT tech on duty.

---

### Q18. You see an unfamiliar error while helping with a computer. What should you do?

**Type:** Multiple choice  
**Category:** Workflow and Escalation  
**Points:** 7

A. Keep clicking until something works  
B. Restart the process repeatedly without telling anyone  
C. Document the error and ask your mentor  
D. Ignore it  

**Correct Answer:** C

**Explanation:**

Unfamiliar errors should be documented and escalated. This helps the mentor understand what happened and prevents accidental mistakes.

---

### Q19. A user asks you a question and you do not know the answer. What should you do?

**Type:** Multiple choice  
**Category:** Workflow and Escalation  
**Points:** 8

A. Guess  
B. Tell them IT cannot help  
C. Ask your mentor or supervisor and document the question if needed  
D. Avoid the user  

**Correct Answer:** C

**Explanation:**

It is better to ask for help than to guess. A strong work-study student knows when to involve a mentor or supervisor.

---

### Q20. What should good ticket notes or mentor summaries include?

**Type:** Multiple choice  
**Category:** Workflow and Escalation  
**Points:** 8

A. "Fixed"  
B. Only the user's name  
C. The issue, what was checked, what happened, and what still needs to be done  
D. Nothing unless the issue is serious  

**Correct Answer:** C

**Explanation:**

Good ticket notes or mentor summaries should explain the issue, what was checked, what happened, and what still needs to be done. New students may draft this information for a mentor instead of updating the ticket/work-order system directly.

---

## Section 6: Work Style and Role Alignment

These questions are separate from technical readiness scoring. They help a supervisor answer: Which environment would help this student succeed fastest?

Role alignment signals:

- Technical troubleshooting interest
- User-facing support interest
- Process/documentation strength
- Ambiguity readiness
- Help-seeking readiness
- Structured shadowing need

### RA1. You are given a task, but one step is unclear. What would you most likely do?

**Type:** Multiple choice
**Category:** Work Style and Role Alignment
**Points:** Separate role-alignment signal

A. Guess and keep going so I do not slow anyone down
B. Stop and ask a specific clarification question
C. Wait silently until someone checks on me
D. Skip the unclear step

---

### RA2. A user is frustrated and says, "IT never fixes anything." What is the best beginner-safe response?

**Type:** Multiple choice
**Category:** Work Style and Role Alignment
**Points:** Separate role-alignment signal

A. Tell them another IT person probably forgot
B. Promise the issue will be fixed today
C. Stay calm, avoid blame, and tell them you will check with your mentor
D. Ignore the comment

---

### RA3. Which type of work sounds most interesting to you?

**Type:** Multiple choice
**Category:** Work Style and Role Alignment
**Points:** Separate role-alignment signal

A. Figuring out why a computer, monitor, printer, or account is not working
B. Helping students or users understand a process, form, appointment, or next step
C. Organizing information, checking records, and making sure details are correct
D. I am not sure yet

---

### RA4. Which task would you feel most comfortable learning first?

**Type:** Multiple choice
**Category:** Work Style and Role Alignment
**Points:** Separate role-alignment signal

A. Checking cables, asset tags, device names, and error messages
B. Greeting students or users and helping direct them to the right person
C. Reviewing a checklist and making sure each step is complete
D. Watching someone else first before choosing

---

### RA5. A mentor asks you to document what happened during a task. What would you include?

**Type:** Multiple choice
**Category:** Work Style and Role Alignment
**Points:** Separate role-alignment signal

A. Only "done"
B. What I checked, what happened, and what I still need help with
C. My opinion about who caused the problem
D. Private information, just in case it helps

---

### RA6. Which situation sounds most stressful to you?

**Type:** Multiple choice
**Category:** Work Style and Role Alignment
**Points:** Separate role-alignment signal

A. A computer problem where I do not know the answer yet
B. A frustrated person asking for help
C. A task with many small details that must be accurate
D. A quiet task where I have to work independently

---

### RA7. Which statement best describes you right now?

**Type:** Multiple choice
**Category:** Work Style and Role Alignment
**Points:** Separate role-alignment signal

A. I want to learn technical troubleshooting, even if it feels challenging
B. I mostly want to help people navigate processes and get answers
C. I like organized tasks with clear steps and records
D. I am still exploring what kind of work fits me

---

## Recommended Alignment Outputs

Recommended Alignment: IT Launchpad
Student appears interested in technical troubleshooting and shows safe beginner habits. Begin with IT foundations, hardware basics, escalation rules, and mentor-supervised tasks.

Recommended Alignment: Hybrid IT/User Support
Student may benefit from supervised IT exposure while also using communication strengths in user-facing support. Begin with shadowing, documentation practice, and structured beginner-safe IT tasks.

Recommended Alignment: Student Services Exploration
Student shows stronger current alignment with user-facing process support than technical troubleshooting. Admissions, Advising, front desk support, or another structured student services role may match the student's current strengths.

Recommended Alignment: Structured Shadowing
Student may benefit from observing IT, Admissions, and Advising workflows before final placement. Focus on confidence, communication, and task preference discovery.

---

# Training Path Recommendation Logic

## Beginner: 0-39

Recommended path:

1. Common IT Words
2. What Does IT Do?
3. What Is a Domain?
4. What Is SCCM / Configuration Manager?
5. What Is Imaging?
6. What Is a Ticket?
7. Hardware Basics
8. How to Ask for Help
9. Day 1 Checklist
10. Shadowing

## Developing: 40-69

Recommended path:

1. Day 1 Checklist
2. Common IT Words Review
3. Hardware Basics
4. Imaging Overview
5. Draft Ticket Notes
6. Escalation Rules
7. Shadowing

## Ready to Shadow: 70-84

Recommended path:

1. Day 1 Checklist
2. Department Expectations
3. Confidentiality
4. Imaging Overview
5. Escalation Rules
6. Shadowing

## Advanced Beginner: 85-100

Recommended path:

1. Day 1 Checklist
2. Required Department Expectations
3. Confidentiality
4. Imaging Overview
5. Escalation Rules
6. Scenario Practice
7. Mentor Check-In

---

# Knowledge Gap Mapping

Use these rules to recommend extra modules.

| Weak Area | Suggested Modules |
|---|---|
| Low IT Vocabulary | Common IT Words, Domain Basics, Imaging Overview, Configuration Manager Overview |
| Low Beginner Troubleshooting | Hardware Basics, Cables and Peripherals, Beginner Troubleshooting Workflow |
| Low Workflow and Escalation | How to Ask for Help, Escalation Rules, Ticket Notes Basics |
| Low Confidence | Extra Shadowing, Mentor Check-In, How to Read a Procedure |
| No Prior IT Experience | Full IT Foundations path |

---

# Supervisor Summary Template

Student:
Pre-Assessment Score:
Starting Level:
Recommended Path:

## Knowledge Gaps

- Low IT vocabulary:
- Low beginner troubleshooting:
- Low workflow and escalation:
- Low confidence:
- No prior IT experience:

## Strengths

- Prior technology exposure:
- Customer service or communication strengths:
- Strong confidence areas:
- Strong scenario responses:

## Confidence Notes

- Student concerns:
- Topics the student feels comfortable with:
- Questions to revisit during first mentor check-in:

## Mentor Recommendation

- Recommended starting path:
- Modules to prioritize:
- Shadowing focus:
- First-week support plan:

## Suggested First Shadowing Focus

- Vocabulary support:
- Hardware observation:
- Imaging observation:
- Ticket/work-order observation:
- Asking-for-help practice:
